...
<body class="bg-gray-50 min-h-screen flex flex-col items-center p-4">
  ...
  <!-- PAYWALL SIN MENSAJE WHATSAPP + AUTO ACTIVA -->
  <div id="paywall" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.7); z-index:9999; align-items:center; justify-content:center; padding:20px;">
    <div id="paywall-box" style="background:white; border-radius:20px; padding:24px; max-width:360px; width:100%; text-align:center; font-family:sans-serif;">
      ...
      <a onclick="autoActivarPro()" href="https://mpago.la/2LprVEr" target="_blank" style="display:block; width:100%; background:#2563eb; color:white; padding:14px; border-radius:12px; font-weight:bold; margin-top:20px; text-decoration:none; text-align:center;">PRO MES - $49/mes</a>
      <a onclick="autoActivarPro()" href="https://mpago.la/28kWMCz" target="_blank" style="display:block; width:100%; background:#000; color:white; padding:14px; border-radius:12px; font-weight:bold; margin-top:12px; text-decoration:none; text-align:center;">PRO AÑO - $249/año (Ahorras 60%)</a>
...
function autoActivarPro(){
  // Se activa solo al volver del pago
  localStorage.setItem('esPro','si');
  setTimeout(()=>{
    document.getElementById('paywall-box').innerHTML = '<h2 style=\\"font-weight:bold;font-size:20px\\">¡Pago recibido! 🎉</h2><p style=\\"margin-top:10px;color:#666\\">Ya eres PRO. Cierra Mercado Pago y recarga la página.</p>';
  }, 1000);
}
